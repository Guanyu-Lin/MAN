...
def mergeSort(loc, arr, l, r):
	if l < r:
		...
		mergeSort(loc, arr, l, m)
		mergeSort(loc, arr, m+1, r)
		merge(loc, arr, l, m, r)
...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        num_groups=flags["num_groups"] if "num_groups" in flags else None,
...
def dcg_score(y_true, y_score, k=10):
    ...
    # order = [i for i in range(len(y_score))]
    # mergeSort(order, y_score, 0, len(y_score)-1)
    order = np.argsort(y_score, kind='heapsort')[::-1]
    # print("y_true", y_true, "y_score", y_score, "order", order)
    # print(order)
